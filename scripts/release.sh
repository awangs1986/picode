...
# Cargo.lock contains a `picode` entry whose version tracks Cargo.toml.
...
if [[ -n "$CARGO_BIN" ]]; then
  ( cd src-tauri && "$CARGO_BIN" update -p picode --offline >/dev/null 2>&1 \
      || "$CARGO_BIN" update -p picode >/dev/null 2>&1 \
      || true )
else
