...
微信消息通过现有 Pi TUI Authority 作为真实 user message 进入当前对话，效果等同于用户在
TUI 输入同样的文字；它使用当前对话、当前模型、当前权限和当前工具完成正常模型回合。
完整助手回答保留在 TUI。随后使用当前 TUI 已选模型和账号做一次不写入对话历史、无工具的
微信精简调用，只把精简后的最终文本发回微信。
...
- 同一传输按顺序处理消息；Pi 正在运行其他回合时等待并重试，不会创建第二个 Runtime。
- 入站模型回合或微信发送暂时失败时不会提前消费消息；发送重试不会重复执行已完成的 Pi 回合。
- iLink 返回 `errcode=-14` 时立即停止失效 Poller 并要求重新登录，不再错误显示为健康运行。
...
`context_token`、Vault 引用、单对话绑定、停止和禁用，以及 TUI 回合临时失败重试、微信发送重试、
会话过期停止和一次独立的微信回复精简。真实扫码仍需要用户自己的微信做一次手工 Gate。
